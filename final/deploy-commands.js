...
try {
    ...
    // --- BURAYA EKLENEN YENİ KONTROL KODU ---
    const commandNames = new Set();
    const duplicateCommands = [];

    commands.forEach(command => {
        if (commandNames.has(command.name)) {
            duplicateCommands.push(command.name);
        } else {
    ...
    if (duplicateCommands.length > 0) {
        console.error('[HATA] ❌ Tekrarlanan komut adları bulundu. Lütfen bunları düzeltin.');
        console.error('[TEKRAR EDEN KOMUTLAR] ➡️', duplicateCommands.join(', '));
        // Tekrarlanan komutlar varsa, programı durdur
        process.exit(1);
    }
    // --- YENİ KONTROL KODUNUN SONU ---
